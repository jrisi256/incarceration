...
out_dir <- file.path("nosrati_replication", "output", "descriptives")
...
pdf(here(out_dir, "density_plots.pdf"), onefile = T)
...
ggsave(here(out_dir, "nr_obs_year.pdf"))
...
ggplot(nr_obs_county, aes(x = n)) +
  ...
  theme_bw()
ggsave(here(out_dir, "nr_obs_county.pdf"))
...
pdf(here(out_dir, "change_over_time.pdf"), onefile = T)
pmap(
  list(
...
ggplot(data = data) +
  ...
  theme_bw()
ggsave(here(out_dir, "time_drugs.jpg"))
...
ggsave(here(out_dir, "scatterplot_matrix.jpg"))
...
ggplot(data = data_mean_sd, aes(x = income__mean, y = drugs__mean)) +
  ...
  theme_minimal()
ggsave(here(out_dir, "income_drugs.pdf"))
...
ggplot(data = data_mean_sd, aes(x = income__mean, y = county_jail_adm__mean)) +
  ...
  theme_minimal()
ggsave(here(out_dir, "income_jail.pdf"))
...
ggplot(data = data_mean_sd, aes(x = income__mean, y = county_prison_adm__mean)) +
  ...
  theme_minimal()
ggsave(here(out_dir, "income_prison.pdf"))
...
ggplot(data = data_mean_sd, aes(x = county_jail_adm__mean, y = drugs__mean)) +
  ...
  theme_minimal()
ggsave(here(out_dir, "jail_drugs.pdf"))
...
ggplot(data = data_mean_sd, aes(x = county_prison_adm__mean, y = drugs__mean)) +
  ...
  scale_y_continuous(limits = c(0, 20), breaks = seq(0, 20, 4)) +
  theme_minimal()
ggsave(here(out_dir, "prison_drugs.pdf"))
...
pdf(here(out_dir, "prescription_scatterplots.pdf"), onefile = T)
pmap(
  list(
    as.list(select(
...
pdf(here(out_dir, "histograms_cov.pdf"), onefile = T)
pmap(
  list(
